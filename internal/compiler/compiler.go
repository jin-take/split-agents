...
func (c *Compiler) Plan(ctx context.Context, prompt string, wantTitle bool) domain.Plan {
	p := heuristic(prompt)
	if !wantTitle && len([]rune(prompt)) < 140 && !looksComplex(prompt) {
		return p
	}
	...
	text, _, err := c.AI.Complete(ctx, "gpt-5.6-luna", "low", inst, prompt, 450)
	if err != nil {
		return p
	}
	text = strings.TrimSpace(strings.TrimSuffix(strings.TrimPrefix(strings.TrimSpace(text), "```json"), "```"))
	var x domain.Plan
	if json.Unmarshal([]byte(text), &x) != nil {
		return p
	}
	x.PlannerUsed = true
	if !wantTitle {
...
func heuristic(prompt string) domain.Plan {
	p := domain.Plan{TaskSummary: prompt, Complexity: "normal", Model: "gpt-5.6-terra", Reasoning: "low", Subagents: 0, RecentMessages: 6}
...
func looksComplex(s string) bool {
	l := strings.ToLower(s)
	for _, k := range []string{"architecture", "設計", "調査", "research", "review", "複数", "比較", "実装", "debug", "原因", "security"} {
...
func normalize(p *domain.Plan) {
	...
	if p.RecentMessages < 2 {
		p.RecentMessages = 2
	}
	if p.RecentMessages > 10 {
		p.RecentMessages = 10
	}
	if p.Subagents < 0 {
		p.Subagents = 0
